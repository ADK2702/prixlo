...
echo Fixing FTS index (unaccent_immutable wrapper)...
...
"%PGBIN%\psql.exe" -U %PGUSER% -d %PGDB% -c "SELECT name, price, merchant FROM v_active_prices WHERE name ILIKE '%%lait%%' ORDER BY price LIMIT 5;"
echo.
pause
